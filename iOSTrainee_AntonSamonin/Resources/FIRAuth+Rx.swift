...
extension Reactive where Base: Auth {
    ...
    /**
     ...
     @remarks See `FIRAuthErrors` for a list of error codes that are common to all API methods.
     */
    public func signInWithResult(withEmail email: String, password: String) -> Observable<AuthDataResult> {
        return Observable.create { observer in
    ...
    public func signIn(withEmail email: String, password: String) -> Observable<Bool> {
        return Observable.create { observer in
            self.base.signIn(withEmail: email, password: password) { auth, error in
                ...
                if error != nil {
                    ...
                    print("Запрос входа не выполнен.")
                }
            }
            return Disposables.create()
        }
